...
def send_email_reminder(self):
    ...
    two_hour_ahead = current_time + timedelta(hours=2)
    ...
    appointments_one_day = ClientAppointment.objects.filter(
        ...
        appointment_time__hour=two_hour_ahead.time().hour,
        appointment_time__minute=one_day_ahead.time().minute
    )

    # Find out appointment 1 hour ahead
    appointments_two_hour = ClientAppointment.objects.filter(
        appointment_date=two_hour_ahead.date(),
        appointment_time__hour=two_hour_ahead.time().hour,
        appointment_time__minute=two_hour_ahead.time().minute,
    ...
    for appointment in appointments_two_hour:
        email_subject = 'Reminder: Property inspection.'
        email_message = f"Hello {appointment.first_name}, kindly note that your appointment is about to start in 2 hours from now, location:{appointment.selected_property_address} see you there!"
        sender = 'Favour Idowu'
        email_address = appointment.email_address
        custom_email_sender(email_address, email_subject, email_message, sender)
